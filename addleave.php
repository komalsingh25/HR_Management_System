...
<?php
  ...
  $candid = $_SESSION['User']['CandidateId'];
  $sql = mysqli_query($db,"select * from type_of_leave ");
  $registrationdetails = mysqli_query($db,"select * from registrationdetails where CandId='$candid' ");
  ...
  if(isset($_POST['Apply']))
  {
    ...
    $datee = str_replace('/', '-', $interviewday);
    $endd = date('Y-m-d', strtotime($datee));
    /*end date formate*/

    mysqli_query($db,"insert into registrationdetails values(null,'$candid','$leave','$reason','$startd','$endd','$status')");
...
<div class="container-fluid" style="margin-bottom: 30px;margin-top: 10px; background: white;">
	<div class="row">
        ...
        <div class="input-group">             
            ...
            <option value="<?php echo $row['RegistrationId']; ?>"><?php echo $row['Technology_Name'];?></option>
            <?php } ?>
            </select>
        </div>
        </div>
        ...
        <div class="col-md-5 control-label">
            ...
            <div class="input-group">             
                <span class="input-group-addon">
              	...
              	<input type="text" id="InterviewDay" name="interviewday" title="Select End Date" class="form-control" required="" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')">
            </div>
        </div>
        <div class="clearfix"> </div>
    </div>
...
<div class="validation-form" style="margin-bottom: 30px;margin-top: -10px; background: white; ">
    ...
    <?php $i=1; while($rom = mysqli_fetch_assoc($registrationdetails)) {
      $typeid = $rom['TechId'];
      $typename = mysqli_query($db,"select * from type_of_leave where RegistrationId='$typeid' ");
    ...
    <div class="row" style="margin-right: 0; margin-top: 10px; margin-left: 0;">
    	...
    	<div class="col-md-2"><?php echo ucfirst((isset($typen['Technology_Name']))?$typen['Technology_Name']:""); ?></div>
    	<div class="col-md-2" ><?php echo(isset($rom['Reason']))?$rom['Reason']:""; ?></div>
    	<div class="col-md-2"><?php echo(isset($rom['LastDay']))?$rom['LastDay']:""; ?></div>
    	<div class="col-md-2"><?php echo(isset($rom['InterviewDay']))?$rom['InterviewDay']:""; ?></div>
    	<div class="col-md-1"><?php echo(isset($rom['Status']))?$rom['Status']:""; ?></div>

      <?php if($rom['Status'] == "Pending") {?>
