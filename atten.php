	...
	include_once('controller/connect.php');
  	...
  	if (isset($_POST['yearss']))
  	{
  		...
  		if($by == 3)
  		{
	  		...
	  		$SelectedSortById = $by;
	  		$daty = $_POST['years'];
	  		$viewatten = mysqli_query($db,"SELECT *,COUNT(CandId) as TotalDays,sum(DailyWorkingminutes) as TotalMinute,(SELECT Concat(FirstName,' ',MiddleName,' ',LastName) from candidate where candidateId=d.CandId) as FullName,SUBSTR(LoginDate,1,7) FROM dailyworkload d where SUBSTR(d.LoginDate,1,4)='$daty' group by CandId");
  		}
  		else if($by == 2)
  		{
  			$by = $_POST['by'];
	  		...
	  		$viewatten=mysqli_query($db,"SELECT *,COUNT(CandId) as TotalDays,sum(DailyWorkingminutes) as TotalMinute,(SELECT Concat(FirstName,' ',MiddleName,' ',LastName) from candidate where candidateId=d.CandId) as FullName,SUBSTR(LoginDate,1,4) FROM dailyworkload d where SUBSTR(d.LoginDate,1,7)='$da' group by CandId");
  		}
  		else
  		{
  			$viewatten = mysqli_query($db,"SELECT *,COUNT(CandId) as TotalDays,sum(DailyWorkingminutes) as TotalMinute,(SELECT Concat(FirstName,' ',MiddleName,' ',LastName) from candidate where candidateId=d.CandId) as FullName FROM `dailyworkload` d group by CandId");
  		}
...
<div class="validation-form" style="margin-bottom: 30px;">
	...
	<div class="row" style="margin-right: 0; margin-left: 0;">
		...
		<div class="col-md-2">
			<label><?php echo(isset($row['CandId']))?$row['CandId']:"";?></label>
		</div>
		<div class="col-md-2">
