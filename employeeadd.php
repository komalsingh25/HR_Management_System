...
<?php 
  ...
  {
    $candid = $_GET['candid'];
    $editcandid = mysqli_query($db,"SELECT e.*,ss.StateId,cc.CountryId FROM candidate e join city c on e.CityId=c.CityId join state ss on c.StateId=ss.StateId join country cc on cc.CountryId=ss.CountryId where CandId='$candid'");
    $editemp = mysqli_fetch_assoc($editcandid);
    if(isset($editemp["CountryId"])) {
      $CountryId = $editemp["CountryId"];
  }  
  if(isset($editemp["StateId"])) {
    $StateId = $editemp["StateId"];
  }
  if(isset($editemp["CityId"])) {
    $CityId = $editemp["CityId"];
  }
}  
?>
<ol class="breadcrumb" style="margin: 10px 0px ! important;">
    <li class="breadcrumb-item"><a href="Home.php">Home</a><i class="fa fa-angle-right"></i>Candidate<i class="fa fa-angle-right"></i> Candidate Add</li>
</ol>
<!--grid-->
 	...
 	<!---->
        <form method="POST" action="controller/employee.php?empedit=<?php echo isset($_GET['candid']) ? $_GET['candid'] : ''; ?>" enctype="multipart/form-data">
        <?php 
          if($result)
          {
        ...
        <div class="vali-form-group">
          <div class="col-md-4 control-label">
              ...
              <input type="text" name="candid" title="Employee ID" value="<?php echo(isset($editemp["CandidateId"]))?$editemp["CandidateId"]:""; ?>" class="form-control" placeholder="Candidate ID" required="">
              </div>
            </div>
